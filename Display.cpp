...
BITMAP g_bitmapInfo = {};
POINT g_imagePosition = { 0, 0 };
bool g_isDragging = false;
POINT g_dragOffset = { 0, 0 };
std::vector<POINT> g_shapePixels;

// Цвет фигуры — ЧЕРНЫЙ
COLORREF g_currentShapeColor = RGB(0, 0, 0);
...
// Отображаем имя "Филлерских Захар"
void DrawTextOnImage(HDC hdcMem) {
    ...
    std::string text = "Филяевских Захар";
    SIZE sz;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        if (g_hBitmap)
        {
            ...
            GetClientRect(hWnd, &rect);
            // Центр по X, низ по Y
            g_imagePosition.x = (rect.right - g_bitmapInfo.bmWidth) / 2;
            g_imagePosition.y = rect.bottom - g_bitmapInfo.bmHeight - 20;
            PrepareBuffer(hWnd);
        }
        break;
